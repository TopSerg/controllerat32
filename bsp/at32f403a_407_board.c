...
gpio_type *led_gpio_port[LED_NUM]        = {LED1_GPIO, LED2_EXT_GPIO, LED3_GPIO};
uint16_t led_gpio_pin[LED_NUM]           = {LED1_PIN, LED2_EXT_PIN, LED3_PIN};
crm_periph_clock_type led_gpio_crm_clk[LED_NUM] = {LED1_GPIO_CRM_CLK, LED2_EXT_GPIO_CRM_CLK, LED3_GPIO_CRM_CLK};
...
void at32_board_init()
{
  ...
  at32_led_init(LED1);
  at32_led_init(LED2_EXT);
  at32_led_init(LED3);
  at32_led_off(LED1);
  at32_led_off(LED2_EXT);
  at32_led_off(LED3);
...
/**
  ...
  * @retval none
  */
__always_inline void at32_led_on(led_type led)
{
  if(led > (LED_NUM - 1))
    return;
  if(led_gpio_pin[led])
