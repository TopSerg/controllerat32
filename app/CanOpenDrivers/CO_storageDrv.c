...
static ODR_t storeBlank(CO_storage_entry_t *entry, CO_CANmodule_t *CANmodule) {
	...
	if (flash_write((uint32_t)entry->addrNV, (uint16_t*)entry->addr, entry->len) != SUCCESS)
	{
...
static ODR_t restoreBlank(CO_storage_entry_t *entry, CO_CANmodule_t *CANmodule) {
	...
	memset(&odBuff, 0xFF, sizeof(odBuff));
	if (flash_write((uint32_t)entry->addrNV, (uint16_t*)&odBuff, sizeof(odBuff)) != SUCCESS)
	{
		ret = CO_ERROR_DATA_CORRUPT;
	}
	return ret;
	...
	uint32_t *storageInitError)
{
	...
	*storageInitError = 0;
	for (uint8_t i = 0; i < entriesCount; i++) {
		...
		uint32_t testMagic = 0x87654321;

		memcpy(&odBuff, entry->addrNV, sizeof(odBuff));
		
		if (odBuff.x21FF_magicValue != testMagic)
		{
			if (flash_write((uint32_t)entry->addrNV, (uint16_t*)entry->addr, entry->len) != SUCCESS)
			{
		...
		else /*restore OD data form NVM*/
		{
			memcpy(entry->addr,&odBuff,entry->len);
