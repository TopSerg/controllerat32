...
static CO_CANmodule_t *CANModule_local =
    NULL; /* Local instance of global CAN module */
	...
	uint16_t CANbitRate)
{
	...
	CANmodule->useCANrxFilters =
	    (rxSize <= 4U) ? true : false; /* microcontroller dependent */
...
CO_ReturnError_t CO_CANsend(CO_CANmodule_t *CANmodule, CO_CANtx_t *buffer)
{
	...
	if (canSend(buffer->ident&CANID_MASK, buffer->DLC, buffer->data)) {
...
void reciveProcCanOpen(uint16_t ident, uint8_t dlc, uint8_t *data)
{
	CO_CANrxMsg_t rcvMsg; /* pointer to received message in CAN module */
	rcvMsg.ident = ident;
	rcvMsg.dlc = dlc;
	memcpy(rcvMsg.data, data, sizeof(rcvMsg.data));
	...
	if (CANModule_local->useCANrxFilters) {
		 ...
		 * CAN-ID. */
		buffer = &CANModule_local->rxArray[0];
		for (index = CANModule_local->rxSize; index > 0U; index--) {
			if (((rcvMsgIdent ^ buffer->ident) & buffer->mask) == 0U) {
				msgMatched = true;
				break;
			}
	...
	if (CANmodule->CANtxCount > 0U) {
		...
		for (i = CANmodule->txSize; i > 0U; i--) {
			...
			if (buffer->bufferFull) {
				...
				CANmodule->bufferInhibitFlag = buffer->syncFlag;
				canSend(buffer->ident&CANID_MASK, buffer->DLC, buffer->data);
				break; /* exit for loop */
			}
			buffer++;
		} /* end of for loop */
